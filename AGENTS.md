...
All 225 operations across the ~46-service per-SDK layer are generated. Hand-written code is limited to infrastructure:
...
`spec/basecamp.smithy` holds 225 worked operations. Copy the nearest one rather than
...
**Pin semantics.** The pin is the conformance baseline as of the last sync — it asserts that all upstream drift up to that revision has been *triaged*, not that every contract in it has been *absorbed*. Upstream contracts shipped past the SDK's modeled surface are tracked in `spec/api-gaps/` (status `addressed-in-bc3-pr-N`) until an absorption PR lands. A repin is valid exactly when every drift item in `pin..HEAD` is either absorbed into the spec or registered in `spec/api-gaps/`; it is not blocked on absorption itself. The converse also holds: absorbing a single post-pin upstream contract does not require a repin — the pin is a triage floor, not a ceiling on the modeled surface. Such absorption-ahead-of-the-pin PRs record their upstream anchor in `spec/api-gaps/`, and the next sync's repin finds them already absorbed. The pin never moves backward. The `compatibility.*` pins mark the last **verified API-surface state** of their branch, not a last-glanced timestamp — refresh one only when re-verifying that branch's API surface, and record verification dates in the PR that did the checking.
