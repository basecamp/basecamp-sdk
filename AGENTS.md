...
All `270` operations across the ~50-service per-SDK layer are generated. Hand-written code is limited to infrastructure: <!-- @operation-count -->
...
`spec/basecamp.smithy` holds `270` worked operations. <!-- @operation-count --> Copy the nearest one rather than
