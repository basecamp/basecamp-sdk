...
Two items the SDK canary surfaced. As of the BC5 launch (2026-05-27) both
remain **open** — an earlier draft of this file framed them as pending a
*decision* when they are actually pending a *merge*:

1. `memories` going to `[]` on `GET /my/readings.json` — **confirmed live on
   BC5 `master`** (source-diffed against `four`, which still populates it).
   Filed as
...
2. Longstanding `app_todoslists_url` typo at
   ...
   unshipped**: production `master` still carries only the typo. Impact is nil
   for SDK consumers — the spec models the correctly-spelled `app_todolists_url`,
   which has always read `null`, so there is no regression either way.

Both are tracked in the BC3 plan's Phase 2 compat verification and this SDK
plan's §8.
