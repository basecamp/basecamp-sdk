...
all `267` operations: the 130 GETs are retryable by method, and 91 mutations are flagged <!-- @operation-count -->
...
`SpotlightRecording`, `RecordProjectVisit`, `CreateBubbleUp`, `EnableProjectClients`). The other 44 POSTs are attempted exactly once. SPEC.md §7 specifies the
