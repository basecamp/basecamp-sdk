...
all 243 operations: the 124 GETs are retryable by method, and 79 mutations are flagged
`idempotent: true` — all 48 PUTs, all 24 DELETEs, and 7 POSTs (`CompleteTodo`, `PauseQuestion`,
`SubscribeToCardColumn`, `Subscribe`, `EnableCardColumnOnHold`, `CreateBookmark`, `PrioritizeAssignment`). The other 40 POSTs are attempted exactly once. SPEC.md §7 specifies the
