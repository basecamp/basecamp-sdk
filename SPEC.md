...
All 235 operations in `behavior-model.json` use `retry_on: [429, 503]`. Three `(max, base_delay_ms)` patterns exist:
...
- Idempotent: 75 (flagged with `idempotent: true`)
- Non-idempotent: 160 (no `idempotent` field, or not present)
