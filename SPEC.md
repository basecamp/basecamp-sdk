...
4. Validate `max_retries ≥ 1`. → `⊥ BasecampError(code: "usage")` otherwise. (`max_retries` is total attempts including the initial request; 0 would mean no request is made.) **Divergence:** the SDKs handle `max_retries = 0` in two distinct outcomes across four implementations, neither of which is the spec's `⊥`:
   ...
   - **Kotlin:** the builder rejects a *negative* value (`require(maxRetries >= 0)`) and accepts `0`, which the transport floors to a single attempt on every path, governed or not.
...
1. **No hand-written wire I/O.** Every request flows through public generated wire methods (Go: through the shared generated-client transport). No manual path construction or verb selection. Bodies use the generated request types, with one Go-specific carve-out: where the wire contract is inexpressible through the generated request type, the method MAY marshal an explicit body map and call the operation's generated `*WithBody` variant, with keys matching the generated request schema — the generated wrapper still owns path, verb, content type, and response decoding, and the operation identity still reaches hooks and retry. Two instances of inexpressibility exist: always-send-empty semantics (an explicit clear — an empty string, an empty list — has to reach the wire, and `omitempty` drops it), and a nested struct whose zero values would leak onto the wire as real fields. This is the only sanctioned use of hand-marshaled bodies; a body the generated request type can express keeps using it.
