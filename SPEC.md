...
| `doWithRetry`, **`go/templates/client.tmpl`** → `go/pkg/generated/client.gen.go` | `c.Logger.Debug("request failed", …, "error", err)` — the raw transport error into a structured log line — and `return nil, err` hands the same `*url.Error` to every generated service unwrapped | returns it as-is; nothing projects or replaces it |
