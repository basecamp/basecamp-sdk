...
| Python | Ported. `recordings.summarize(bucket_id=, recording_id=, event_type=, recording_type=)` (`python/src/basecamp/services/recordings.py`), the mention helpers `mentioned_person_ids` / `person_id_from_sgid` / `mention_markup` / `with_mentions` (`python/src/basecamp/mentions.py`), and `comments.expand_mentions` / `create_with_mentions` (`python/src/basecamp/services/comments.py`). Placement is rule 5's Python extension point: service subclasses re-exported by `client.py` and `async_client.py`, sync and async. The projection is a `RecordingSummary` `TypedDict` — the dict shape every other Python read returns — and `updated_at` is the API's own string, not a parsed instant. The composite's refusals are their own exception types (`NoRecordingTypeError`, `UnknownRecordingTypeError`, `RecordingUnresolvedError`, `CampfireDiscoveryIncompleteError`, `BucketMismatchError`, `CampfireIndexLoadAbortedError`) whose `code` sits outside `ErrorCode` on purpose: none of them is an HTTP answer. The Campfire index lives on the `Client` as `campfire_index`, dock-then-listing, ten minutes each with a 30 s refresh floor, single-flight per key and bounded by entry count. Conformance-covered per rule 4: the Python runner executes every case in `conformance/tests/recording_summary.json`. **One divergence, in payload typing.** Go decodes the discovery reads into `int64` fields, so an `id` arriving as `true`, `"7"` or `7.0` fails the whole decode and the read errors. Python's generated services return `dict`, with nothing typed between the composite and the wire, so it screens ids by TYPE at each site and skips the entry instead — a malformed entry is passed over where Go abandons the read. The VALUE rules are Go's exactly and differ per site (the dock skips `0` and keeps a negative, matching `item.ID != 0`; the account listing screens only the bucket id and lets the campfire id through untouched; the projection passes the id along unchanged), because a candidate dropped here spends no budget and so changes the verdict, not merely the ids. `_check_bucket` is the deliberate exception: an id it cannot read as an integer, or a `bucket` that is not an object, fails CLOSED as a mismatch rather than skipping the check, since `2085958499.0 == 2085958499` is true in Python and would wave a recording from another project through the one check that exists to stop it. |
