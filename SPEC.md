...
| `doWithRetry`, **`go/templates/client.tmpl`** → `go/pkg/generated/client.gen.go` | `c.Logger.Debug("request failed", …, "error", err)` — the raw transport error, straight into a structured log line — and `return nil, err` hands the same `*url.Error` to every generated service unwrapped | returns it as-is; nothing projects or replaces it |
...
| `NetworkError`, `ruby/lib/basecamp/network_error.rb` | `hint: cause&.message` — the transport's own rendering, in the slot the class documents for display; the `message` itself is fixed (`"Connection failed"`) at `http.rb` | `cause: cause` |
| `Client#download_url`, `ruby/lib/basecamp/client.rb` | `NetworkError.new("Download failed: #{e.message}", cause: e)` — here the interpolation is in the message, not the hint | `cause: e` |

Two of those rows need their fix site named, because the obvious file is the wrong one. The generated Go row is fixed in `go/templates/client.tmpl` and never in the emitted `client.gen.go`, per the repository's generated-file rule; and it is the widest row in the table, since every generated service returns that error unwrapped, so it is not a debug-logging footnote. Ruby's row is two constructors, not one, and they leak through different slots: `network_error.rb` puts the transport's rendering in `hint` while keeping a fixed `message`, and `client.rb`'s download path puts it in the message. `hint` counts here despite rule 4's retention carve-out — that carve-out is for a field a caller reaches for deliberately, and `hint` is documented and displayed as guidance, which makes it a default rendering in everything but name.

Naming them is the point of naming them: a contract that reads repository-wide while its only tracked work is one connector is a contract that quietly accrues untracked convergence, which is the failure this section exists to stop. Go's hand-written row is also the one rule 3 turns on — its `Cause` is simultaneously the peer-bearing chain to project out and the chain `shouldTripCircuit` reads through — so the two changes are one change, not two. Tracked in #788 alongside the connector renderings. The other two are lighter but not empty: Swift's `.network` renders a fixed `"Network error"` and *retains* its cause, which rule 4 permits, but that cause is the generic slot `isCancellation` walks, so what it owes is rule 3's projection there rather than a rewording; and TypeScript's main loop composes no peer text, while `typescript/src/download.ts` throws `Errors.network(error.message, error)` at two sites, interpolating the transport's own rendering, and is in scope for the same reason the three above are.
