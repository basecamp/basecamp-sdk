...
| TypeScript, Ruby, Python, Kotlin, Swift, Rust | Not implemented. The constituent reads exist in every SDK, so each can carry the same composition; the fixture — every case in `conformance/tests/recording_summary.json`, the routing matrix and the projection included — is the contract a port implements, and until then each runner rosters all of its cases in `spec/zero-skip-roster.yml` as unwaivered — tracked work, not an accepted divergence. A port deletes its own entries. |
