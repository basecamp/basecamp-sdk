...
**The primitive is the boundary, and one decode currently falls outside it.** `GetPersonProgress` returns a wrapper, and its generated method decodes the wrapper's `person` member *after* the primitive has returned (`swift/Sources/Basecamp/Generated/Services/ReportsService.swift`, `kotlin/…/generated/services/reports.kt`); the same generated code reaches the `events` array through accessors that raise a null-assertion or cast failure, not a decoder exception, on a wrong-shaped wrapper. Those failures still surface raw. It is the only operation where a malformed 2xx body does not reach the shape above, it is generated code in both SDKs, and it is tracked rather than fixed here (#728).
