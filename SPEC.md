...
All 233 operations in `behavior-model.json` use `retry_on: [429, 503]`. Three `(max, base_delay_ms)` patterns exist:
...
- Idempotent: 74 (flagged with `idempotent: true`)
- Non-idempotent: 159 (no `idempotent` field, or not present)
