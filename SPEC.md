...
| `recording_summary.json` | RecordingsSummarize: comment.created routes to one typed read and reports mentioned person ids; every other routable event type and recording type — the full routing matrix — projects type, id, status, title, app_url, content and mentions (23 cases); boost.created refused with no request and its own error identity; a chat line found under the second visible Campfire after the dock 404s; "unresolved" as an identity distinct from a candidate's 403 (28 cases in all) | §18, Appendix F |
