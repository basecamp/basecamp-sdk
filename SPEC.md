...
account, attachments, bookmarks, boosts, bubbleUps, calendars, campfires, cardColumns, cardSteps, cardTables, cards, checkins, clientApprovals, clientCorrespondences, clientReplies, clientVisibility, cloudFiles, comments, documents, drafts, eventFeed, events, everything, folders, forwards, gauges, googleDocuments, hillCharts, lineup, messageBoards, messageTypes, messages, myAssignments, myNotes, myNotifications, people, projects, recordings, reports, schedules, search, subscriptions, subtasks, templates, timeline, timesheets, todolistGroups, todolists, todos, todosets, tools, uploads, vaults, webhooks, wormholes
...
- Go implements `clientVisibility` on `RecordingsService` (not a separate service); uses singular `Timesheet` vs `timesheets`
...
All `273` operations in `behavior-model.json` use `retry_on: [429, 503]`, except `UpdateProjectClientAccess` (`[503]` — its 429 is a seat-limit verdict, §7 Gate 3). <!-- @operation-count --> Three `(max, base_delay_ms)` patterns exist:
...
The SDK also **holds** credentials at rest that never transit a failure path: the bearer token inside its auth strategy, the OAuth `client_secret` in configuration, and §15's webhook signing secret on the receiver. A held value has nothing to project — it appears in no URL, response body, or transport error — and owes only that no rendering is ever built from configuration, which none is: not an error message, a log line, or a hook argument. (A caller who serializes their own configured client or receiver is publishing their own secret; no library rule reaches that act.) Beyond the table and those held values, nothing the SDK handles carries a credential — an inventory audited by sweep, not assembled from review findings, after three rounds each surfaced a missing row. Request URLs carry none — the token is in the header — which is why §12's `RequestInfo.url` and the pagination hooks may carry the full URL, and why hop 2 fires no request hooks (§14). One input can smuggle a credential into hop 1: `downloadURL` accepts any absolute URL and §14 step 2 preserves its query through the origin rewrite, so a caller who passes a signed storage URL puts its signature on hop 1's request. Hop 1's hook arguments are therefore projected, in every SDK (#837): its URL is rendered as origin and path — no userinfo, query or fragment, which is all an observer needs from a download request — and a transport failure reaches `on_request_end`, `on_retry` and the caller as the fixed network error with its cause projected as below, because a transport's rendering of that failure is the same URL. The wire request keeps the whole URL. Ordinary API requests were once left alone on the ground that their URLs carry no credential, with the projection gated on the download flow. That gate held only as long as nothing else could reach the same transport error, and other paths did — a caller sending a signed storage URL through the generic request primitive, the generic path beneath a projected download flow, a discovered OAuth endpoint — so where a transport writes the URL it failed on into its own error, the projection now sits at the one site that turns that error into the SDK's network error, and applies to every request (#869, #871, #872). The rule there: the text is built from the URL the SDK issued, never from the transport's rendering of it. A URL on the API origin carrying no userinfo renders as scheme, host and path — the token rides in the header and the query is paging and filtering; any other URL, a storage host's or a discovered issuer's, renders as its origin alone, since a storage service can sign the path as readily as the query. Never the query, the userinfo or the fragment, in either case. Rust strips the URL from the reqwest error before it becomes hint or source (`without_url`, #869), rendering nothing; Swift rebuilds `URLError` from its code and the failing URL as origin and path, on every host (#871); Kotlin rebuilds Ktor's timeout exceptions around the projected URL and the budget the SDK itself installed (#872). A transport whose error renders no URL — undici, `Net::HTTP`, httpx — has nothing to project and keeps its diagnostic as the cause. Go's ordinary path keeps the gate: hooks and `ErrNetwork`'s hint carry `*url.Error`'s whole rendering, and only hop 1 of a download is projected.
...
- **The write path is closed, with one residual in TypeScript.** The
  ...
  `ParseInt` scan the normalizer uses, at exactly the three fields it writes
  back — `assignees`, `completion_subscribers` and a schedule entry's
  `participants` ([PR #913](https://github.com/basecamp/basecamp-sdk/pull/913)).
  It also answers the two shapes no walk reaches: an absent `id` is `0`, and a
  `null` element is the zero person, as in the reference. An explicit
  `"id": null` still fails the read. Python and Ruby match the reference on
  every measured row. TypeScript does not on the JavaScript number boundary,
  in both directions: an id past 2^53 is REFUSED where the reference reads it,
...
| recording-summary | `recording_summary.json` | §18 Hand-Written Composite Methods, Appendix F (Recording Summaries and Mention Helpers — the composite every runner executes; Go was the reference the ports followed) |
...
- "CreateBubbleUp POST retries when marked idempotent" — GET-only retry (waiver 2B.3).
- "DeleteBubbleUp DELETE retries when marked idempotent" — GET-only retry (waiver 2B.3).
...
**Kotlin** (`kotlin/conformance/.../Main.kt` `KOTLIN_SKIPS`, plus the `link-header` entry below, which comes from the tag branch rather than from the constant):
- "List operation returns first page with Link header" — architectural: the SDK auto-paginates, and its status model reports the last consumed response, so a one-response queue yields "no response" (see the tag-branch discussion above).
- "RecordingsSummarize refuses a wrong-typed parent id, which is not a person's flexible one" — accepting-direction: `kotlinx.serialization` reads a quoted number into any numeric primitive regardless of `isLenient`, so a plain int64 is not strict here until the generator emits a serializer that makes it so (card 43).
- "RecordingsSummarize refuses a wrong-typed id on a parent's bucket" — accepting-direction: the same quoted-number read, one member further in.

**Swift** (`conformance/runner/swift/.../Runner.swift` — `temporarySkips` is empty since the Go-first composite was ported; the one entry below comes from the `link-header` tag branch):
- "List operation returns first page with Link header" — architectural, same as Kotlin: auto-pagination plus a last-consumed-response status model.
...
| `recording_summary.json` | RecordingsSummarize: comment.created routes to one typed read and reports mentioned person ids; every other routed event type — every action the feed catalogues plus the planned `card.moved`, not only `.created`; the planned `*.updated` actions route by subject in Go but are not pinned until bc3 emits them — and every routed recording type, the `Chat::Lines::*` path through Campfire discovery included: the full routing matrix, each projecting type, id, status, title, app_url, content, mentions, the nested bucket/creator/parent/assignee identities and updated_at (34 cases); boost.created refused with no request and its own error identity; a chat line found under the second visible Campfire after the dock 404s; "unresolved" as an identity distinct from a candidate's 403; a truncated Campfire listing as `campfire_discovery_incomplete`; a recording type outside the routing table; a read that answered from another bucket — with the canonical CODE pinned alongside the identity on all five; and the depth of the nested decode (card 39) — a wrong-typed string, bool, company member, parent member, bucket member and parent bucket each failing the read, a malformed member of an assignee PAST THE FIRST failing it too, against three cases pinning the other direction so the rule cannot be satisfied by refusing too much: an unknown key dropped, a `bucket` key ignored on a `TodoParent`, and a person id read as the `FlexibleInt64` it is and EMITTED decoded; and the two id readers kept apart — a parent's and a nested bucket's are plain `int64` where a person's is flexible (55 cases in all) | §18, Appendix F |
...
- Total operations: `273` <!-- @operation-count -->
- Idempotent: 97 (flagged with `idempotent: true`)
- Non-idempotent: 176 (no `idempotent` field, or not present)
...
read/write pair over `<bc-attachment>` sgids, are implemented in all seven
SDKs, Go first as the reference the others ported against:
...
sets them apart from the merge-safe write composites is that they shipped in
...
shape — with the six ports following against the shared fixture, and that
the chat line path carries state (per-bucket dock and per-account listing
caches on the client, ten minutes each) that the other composites do not.

| SDK | Recording summary and mention helpers |
|-----|---------------------------------------|
| Go | `RecordingsService.Summarize(ctx, RecordingRef)` routes on the feed event type (`comment.*`, `message.*`, `todo.*`, `card.*`, `chat.line.*`) or the BC3 recording type to the one typed read that serves it — a deliberate set exposed by `SummarizableRecordingTypes()` / `SummarizableEventTypes()` and held to the README by a test; anything else is `ErrUnknownRecordingType` by design; refuses `boost.*` with `ErrNoRecordingType`; discovers a chat line's Campfire from the bucket's project dock first and the account-wide listing second (each cached ten minutes, refreshed on a miss no more than once per 30 s), under one candidate budget, and reports `ErrRecordingUnresolved` — distinct from any read failure and from `ErrCampfireDiscoveryIncomplete` — only when every visible candidate answered 404. Mentions are read by decoding the sgid envelope structurally (Marshal or JSON, both Rails layouts; unsigned) and parsing the gid whole, and written from `attachable_sgid`. Conformance-covered per §18 rule 4: `conformance/tests/recording_summary.json` (`RecordingsSummarize`, `CommentsCreateWithMentions`), executed by the Go runner, with the projection's shape and the request sequence pinned there and the rest in native Go tests. |
| TypeScript | `client.recordings.summarize(ref)` and `client.comments.expandMentions` / `createWithMentions`, ported from the Go contract against the shared fixture: the same routing tables and deliberate type set (`summarizableRecordingTypes()` / `summarizableEventTypes()`), the same two-source Campfire discovery under the same TTL, refresh floor, candidate budget and listing cap, and the same separation of a candidate's non-404 answer from "under no Campfire you can see". The composites live in `typescript/src/services/recordings-extensions.ts` and `comments-extensions.ts`, wired in `client.ts` (§18 rule 5), with the sgid decoding and `<bc-attachment>` walk in `typescript/src/services/mentions.ts`. The conclusions that are not one read's HTTP answer are `RecordingSummaryError` subclasses carrying a `kind` (`no_recording_type`, `unknown_recording_type`, `recording_unresolved`, `campfire_discovery_incomplete`, `bucket_mismatch`) — matched rather than parsed, and statusless, so a consumer can tell `recording_unresolved` from a 404 a read returned. **Payload typing is reproduced at every field of every nested identity** (`PERSON_STRING_FIELDS` and its siblings in `recordings-extensions.ts`), with four residues this runtime cannot close, and four further differences of a different kind — eight in all, each named below rather than folded into the claim, ahead of the composite's two unrelated deliberate divergences at the end of this row. Card 39 closed it: `nestedIdentity` used to check only that the slot held an object and carry the member verbatim, under a doc comment claiming that a shape Go's decoder refuses fails the read here too — true of the container and false of every field inside it. The table is transcribed from `generated`, so `system_label` stays an ignored unknown key and a `TodoParent`'s `bucket` stays one too. The same change closed the accepting-direction half this port had alone: a person id is the one `FlexibleInt64` in the model and the reference EMITS it decoded, so `"007"` now reaches a caller as `7` rather than as the string every other SDK renders as a number. An adversarial review of that change found the first attempt still short in four places — the int64 window dropped from the number branch of the person-id reader, a parent's id and a nested bucket's id never typed at all, and a `??` fallback that read as if it handled a null assignee and could not be reached — each of which is now closed and pinned. **Residues that stay declared, because this runtime cannot close them:** `7.0` is accepted as `7`, since `JSON.parse` cannot see the fraction that Go's decoder refuses (Ruby and Python both refuse it; this port cannot); `created_at` / `updated_at` inside a person are checked for type and not parsed as instants, the same residue Ruby's row declares; a person id past `Number.MAX_SAFE_INTEGER` is REFUSED where the reference accepts and carries it exactly, because `Number(BigInt("9223372036854775807"))` is 2^63 and handing a caller a person id naming someone else is worse than refusing — deliberately stricter than `summary.id` and `parent.id`, which emit the rounded value under retained waiver 1B.6, and stated as an inconsistency rather than smoothed over; and `9223372036854775807` as a PARENT or BUCKET id is refused for the same parse reason, an over-refusal at one literal value at the top of the int64 window, in the refusing direction. **A fifth, and not this composite's:** `normalizePersonIds` (`typescript/src/services/base.ts`) rewrites the id of any object carrying `personable_type` before a composite sees the body, turning an out-of-safe-range numeric string into `0` plus a `system_label` the reference never emits. Every real person carries `personable_type`, so that pre-pass — not the reader above — governs a realistic body. It is SDK-wide rather than local to the summary and is recorded on card 39 for the person-id work. **A sixth, on a different axis and pre-existing:** an all-zero nested member is emitted as `{}` here where Go's emptiness rule (`Id != 0 || Name != ""`) and Ruby's drop the key; that is the projection's shape rather than its typing, and is not closed by this change. **A seventh and an eighth, on the emission side and both shared with Ruby:** a parent's nested `bucket` is typed — a wrong-typed one fails the read, as the generated decode does — and then emitted whole with its id undecoded, where the reference's conversion builds a `Parent` without a bucket and emits no such key; and an assignee's null `name` is emitted as `null` where the reference emits `""`. A null nested LABEL on the other three members (`creator.name`, `parent.title`, `bucket.name`) is no longer on this list: it was emitted as `null` where Go and Ruby give `""`, and is now written back and pinned. The recording's own unread top-level fields stay declared, identical to Python's and Ruby's. Two further deliberate divergences from Go, neither observable through the fixture: the discovery cache is keyed on the bucket alone and lives on the memoized service, since a TypeScript client is bound to one account and one credential; and the single-flight load has no caller-cancellation attribution to make, because the services take no per-call signal, so a failed load's rejection is simply shared by everyone awaiting it. |
| Ruby | `recordings.summarize(bucket_id:, recording_id:, event_type:, recording_type:)` routes the same deliberate set — exposed by `summarizable_recording_types` / `summarizable_event_types` — to the one generated read that serves it; anything else raises `Basecamp::RecordingRoutingError` (`kind` `unknown_recording_type`, or `no_recording_type` for `boost.*`, before any request). Chat line discovery is the same two sources in the same order — the bucket's project dock, then the account-wide listing — cached on the `Client` (`Basecamp::CampfireIndex`, ten minutes each, refreshed on a miss no more than once per 30 s) and shared by every `AccountClient` it hands out, under the same candidate budget; `Basecamp::UnresolvedRecordingError` is raised only when every visible candidate answered 404, and `Basecamp::CampfireDiscoveryIncompleteError` when candidates were left unsearched. Those identities are error CLASSES carrying `#kind`, not new `ErrorCode` values: §6's table is the HTTP-status taxonomy and has no room for a composite's own verdicts, so each class documents the canonical `code` it reports under. Mentions live in `Basecamp::Mentions`, whose five public helpers are `mentioned_person_ids`, `person_id_from_sgid`, `mention_markup`, `with_mentions` and `bc_attachment_sgids` — the last public here where the reference keeps its equivalent unexported, for a caller deduplicating its own writes. It is `person_id_from_sgid` that decodes the sgid envelope structurally, through a restricted Marshal reader — never `Marshal.load`, which would instantiate arbitrary objects out of rich text other people wrote. `comments.expand_mentions` / `create_with_mentions` resolve every id through `people.get` and deduplicate on the exact `attachable_sgid` that read returned. Placement per §18 rule 5: zeitwerk `prepend` modules (`ruby/lib/basecamp/services/recordings_extensions.rb`, `comments_extensions.rb`) wired by the `on_load` hooks in `ruby/lib/basecamp.rb`, so the accessor's class stays the generated constant exactly. Conformance-covered per §18 rule 4: every case of `conformance/tests/recording_summary.json` executes on the Ruby runner. **Payload typing is reproduced at every field of every nested identity**, not only at the three values the projection reads (`Basecamp::Services::RecordingsExtensions::PERSON_STRING_FIELDS` and its siblings). That was card 39: this port used to type the member, its id and its label and pass the rest through, declaring the remainder as the whole-body-decode limit — a defensible line, and the minority one, since four of the seven SDKs reproduce the whole nested decode for free through a typed decoder and cannot stop short of it. The table is transcribed from `generated`, not from the hand-written wrapper types: the Go service wrapper hands the body to the generated client and converts afterwards, so `system_label` is an unknown key the reference IGNORES and a person id is a `FlexibleInt64` where a parent's and a bucket's are plain `int64`. **The residue that stays declared**, enumerated once on `#project`: the recording's own unread top-level fields — `url`, `visible_to_clients`, `bookmark_url`, `created_at`, `content_attachments` — which the reference decodes whole and this projection never reads, closing which is the whole-recording decoder across every routed type and a different unit of work; `updated_at`, on the recording and on a person, alongside a person's `created_at`, where the type is checked and the instant is not parsed; an assignee's null `name`, emitted as nil where the reference emits `""`, because the label write-back reaches a creator, a parent and a bucket but not `#read_assignees` (TypeScript matches); and a parent's nested `bucket`, typed and then emitted whole with its id undecoded, where the reference emits no such key. **One retained divergence, declared rather than fixed:** the sgid gid's AUTHORITY is parsed by Ruby's `URI`, which is stricter than the reference's `net/url`, so Ruby refuses a mention the reference reads — measured at 7 of 208 hand-built authorities and 14 of 20,000 documents that corrupt one character of a real payload, which is the reachable route, since a corrupted payload decodes to an arbitrary host. Every measured case is now the refusing direction, so it fails closed: a mention is dropped, never misattributed, and on the write side `mention_markup` raises rather than mints a tag for the wrong person. **It was not, until the review of this waiver found otherwise:** the RFC 3986 IPvFuture authority `[v7.x]` resolved in Ruby and is refused by the reference — an accepting-direction case, sitting in the source comment this waiver cites while the waiver asserted none existed. By the rule stated below, that one was CLOSED rather than declared (`Basecamp::Mentions::IPVFUTURE_AUTHORITY`, seven shapes, with the valid IPv6 authorities pinned alongside so the rule cannot be satisfied by refusing too much), and every corpus was then swept for any other accepting row across both stages — 1,917 sgids decoded individually and 33,745 rich-text documents walked, 35,662 in all: none. The residue is a SHAPE rather than a list, and this waiver deliberately does not enumerate it: every attempt to enumerate this same disagreement in the source comment was wrong — a count, then a set of categories, then an absolute that a single shape falsified — and the enumeration that stood here was itself missing an entire class (multi-colon authorities such as `bc3:80:90`, where the reference takes the LAST colon as the port boundary and leaves earlier ones in the hostname, while Ruby refuses the authority outright) until a sibling port measured it. Classes seen so far, as examples and not as a set: raw non-ASCII and `" < > ]` in the host; the IPv6 zone-id; multi-colon authorities; and a malformed percent-escape in the QUERY, which the reference never unescapes and therefore accepts (2 of the 14 query rows in a 64-row sweep, while the fragment is clean at 0 of 27, both sides unescaping it while parsing). The authority on what the residue contains is the differential harness rather than this paragraph; the reference's own accepted sets (host, userinfo, port) were read off it one raw byte at a time and are recorded at `Basecamp::Mentions::HOST_ASCII_ESCAPE`. Closing it means replacing the URL parser, which in a sibling port's first attempt closed 4,167 divergences and opened 1,282 in the ACCEPTING direction — the one outcome worse than the residue, since the write side's only authenticity-adjacent check is whether an sgid names the person it was given. It is therefore its own unit of work, per port, held to zero divergence against the Go oracle in both directions before it lands, and not folded into a change about anything else. Swift has closed it; Ruby has not, and the two differ deliberately until that work is done. |
| Python | Ported. `recordings.summarize(bucket_id=, recording_id=, event_type=, recording_type=)` (`python/src/basecamp/services/recordings.py`), the mention helpers `mentioned_person_ids` / `person_id_from_sgid` / `mention_markup` / `with_mentions` (`python/src/basecamp/mentions.py`), and `comments.expand_mentions` / `create_with_mentions` (`python/src/basecamp/services/comments.py`). Placement is rule 5's Python extension point: service subclasses re-exported by `client.py` and `async_client.py`, sync and async. The projection is a `RecordingSummary` `TypedDict` — the dict shape every other Python read returns — and `updated_at` is the API's own string, not a parsed instant. The composite's refusals are their own exception types (`NoRecordingTypeError`, `UnknownRecordingTypeError`, `RecordingUnresolvedError`, `CampfireDiscoveryIncompleteError`, `BucketMismatchError`, `CampfireIndexLoadAbortedError`), and **identity is the class while `code` stays inside `ErrorCode`** — the split `DeviceFlowError` already uses in `oauth/errors.py`, where the precise outcome lives in `reason` and the coarse code is derived from it, and the one Go uses by construction (sentinel errors with no code slot, matched by its runner with `errors.Is` before any fall-through to `.Code`). §6 declares that enum closed, so a name outside it would be a violation rather than an extension, and `exit_code` maps an unknown code to `ExitCode.API` — which had every one of these reporting "server-side error", including the two refused from the caller's own arguments before any request. Derived per identity: the routing refusals and `bucket_mismatch` are `usage`, `recording_unresolved` is `not_found`, and `campfire_discovery_incomplete` is `usage` and non-retryable (settled for every port on card 40; this port shipped `api_error` and changed). Every verdict is `usage` but `recording_unresolved`, and none is retryable. The Campfire index lives on the `Client` as `campfire_index`, dock-then-listing, ten minutes each with a 30 s refresh floor, single-flight per key and bounded by entry count. Conformance-covered per rule 4: the Python runner executes every case in `conformance/tests/recording_summary.json`. **Payload typing is reproduced at the fields this composite reads, and — since card 39 settled the cross-port fork — at every field of every nested identity. This port already did; Ruby has now closed to it exactly on TYPING, and TypeScript to within the runtime residues its own row names.** That is typing only. On EMISSION this port has not had the pass the other two did, and is now the outlier there: a null nested id reaches a caller as `None` where Go, Ruby and TypeScript give `0`, a null nested label as `None` where they give `""`, and an all-zero creator as `{}` where Go and Ruby drop the key (card 44). Python's generated services return `dict`, so nothing typed sits between the composite and the wire; the port therefore does Go's `json.Unmarshal` work itself at every field this composite reads, and reproduces BOTH of its answers. A `null` is a no-op at any depth — null body, null `dock`, null element, null `id` — yielding the zero value and a read that continues, so refusing it would reject a body the contract accepts. Anything else of the wrong type is a decode error that fails the WHOLE response: an id of `true`, `"7"`, `7.0` or past `int64` fails the read rather than skipping its entry, because a skipped candidate spends none of the discovery budget and so reaches a different VERDICT — "found under no Campfire you can see" where Go says "I could not finish looking". The VALUE rules are Go's per site and differ between them: the dock skips `0` and keeps a negative (`item.ID != 0`); the account listing screens only the bucket id and appends the campfire id with no test, so a null or absent `id` is the candidate `0` and gets its request; the projection passes the id through unchanged. Go decodes the ENTIRE typed model, so a field the composite never reads -- `url`, `visible_to_clients`, `created_at` -- still fails its read and not this one; that residue is the divergence that remains. `_check_bucket` reads its id through the same decode, because `2085958499.0 == 2085958499` is true in Python and an untyped id would otherwise wave a recording from another project through the one check that exists to stop it. |
| Kotlin | Ported. `RecordingsService.summarize(RecordingRef)` and `CommentsService.expandMentions` / `createWithMentions` (`kotlin/sdk/src/commonMain/kotlin/com/basecamp/sdk/services/`), with the mention helpers as top-level functions in `Mentions.kt` — `mentionedPersonIds` / `personIdFromSgid` / `mentionMarkup` / `withMentions`. Placement is the §18 rule-5 extension point for this SDK: the two services are registered in the generator's `EXTENSIBLE_SERVICES` / `HAND_WRITTEN_SERVICES` (`kotlin/generator/.../Config.kt`), so regeneration emits them as `open class` and wires the accessor to the hand-written subclass rather than forking it. Same routing table, same deliberate type set (`summarizableRecordingTypes()` / `summarizableEventTypes()`), same two-source Campfire discovery with the caches on `BasecampClient` — `forAccount` hands out a fresh account client per call, so the index cannot live on one. The error identities are `BasecampException.RecordingSummaryFailure`, whose `reason` carries the fixture's own vocabulary (`no_recording_type`, `unknown_recording_type`, `recording_unresolved`, `campfire_discovery_incomplete`, `bucket_mismatch`) and is the thing to branch on. `reason` is NOT the exception's `code`: §6's table is closed and holds none of those tokens, so `code` is DERIVED from `reason` the way `DeviceFlow` and `DiscoverySelection` already derive theirs — `not_found` for `recording_unresolved`, `usage` for the other four. The conformance runner reads `reason` for `errorType` and answers `errorCode` from the derived `code`, so a fixture pins both and neither can be satisfied by the other; Swift's runner draws the same line through `RecordingSummaryError.canonicalCode`. Executes every case in `conformance/tests/recording_summary.json` on the Kotlin runner but the two nested-decode cases named below, which `KOTLIN_SKIPS` excludes. The nested decode card 39 settled is reproduced here by construction rather than by hand — every routed read decodes the whole typed model — so the top-level residue the three map-based ports declare, fields this composite never reads going unchecked, does not exist in this port: they are decoded, and to the same width as every other field. **But the typed decode is a hair LOOSER than the reference's, in the ACCEPTING direction, and that is this port's real residue:** `kotlinx.serialization` reads a QUOTED NUMBER into any numeric primitive — `"7"` lands in a `Long` with no regard for `isLenient`, which is off here (#598) and does not reach it. The reference refuses that: a `RecordingParent.Id` and a nested `RecordingBucket.Id` are plain `int64` there, and only a `Person.Id` is a `FlexibleInt64`, which is why the flexible case one member over agrees in both. The same lexer serves `Int`, so every integer field this SDK declares outside a flexible serializer — 178 of them (91 `Long`, 81 `Int`, 6 integer lists) across 83 of the 94 generated models, the recording's own fields included — accepts a body the reference rejects, and this composite is only where a fixture finally looked. The two cases that found it are the first entries `KOTLIN_SKIPS` has carried since the port emptied it (§19), rostered rather than left to a green suite; closing it is a strict serializer emitted beside `FlexibleLongSerializer` and applied by the generator to every plain integer field, with its own sweep across the seven runners (card 43), and is not a repair to make inside a change about two other ports. **One further divergence, in the REFUSING direction, recorded rather than closed:** this SDK's generated `Person` declares `system_label`, which the model the reference decodes into does not, so a wrong-typed value there fails the read here and is an ignored unknown key there. Measured from the generated models rather than from a differential run, and left for its own unit of work alongside the other person-id and person-shape questions (card 39, card 34); that one fails toward refusing a body rather than toward misreading one. |
| Swift | Ported (#882 stack). `RecordingsService.summarize(_:)` and the `Mentions` helpers (`swift/Sources/Basecamp/RecordingsServiceExtensions.swift`, `Mentions.swift`, `RubyMarshal.swift`), `CampfireIndex` for the chat-line two-hop (`swift/Sources/Basecamp/CampfireIndex.swift`, held on `BasecampClient` so it outlives the per-call `AccountClient`), and `CommentsService.expandMentions(_:mentioning:)` / `createWithMentions(recordingId:content:mentions:)` (`swift/Sources/Basecamp/CommentsServiceExtensions.swift`). Same-module extensions, §18 rule 5. The composite's own failures are a `RecordingSummaryError` rather than a `BasecampError` case, because a closed HTTP-shaped enum cannot express "under no Campfire you can currently see"; a constituent read that fails still throws its own `BasecampError`. The discovery cache is an actor whose load runs in an unstructured task, so it has no owner-attributed failure to re-run — the one deliberate simplification of the Go cache. Executes every case in `conformance/tests/recording_summary.json`. The nested decode card 39 settled is reproduced here by construction rather than by hand — every routed read decodes the whole typed model — so the top-level residue the three map-based ports declare does not exist in this port. **One divergence in the REFUSING direction, recorded rather than closed:** this SDK's generated `Person` declares `system_label`, which the model the reference decodes into does not, so a wrong-typed value there fails the read here and is an ignored unknown key there. Measured from the generated models rather than from a differential run, and left for its own unit of work alongside the other person-id and person-shape questions (card 39, card 34); it fails toward refusing a body rather than toward misreading one. |
| Rust | Ported (#882 stack). `RecordingsService::summarize(&RecordingRef)` routes on the same deliberate sets, published as `summarizable_recording_types()` / `summarizable_event_types()`; refuses `boost.*`; discovers a chat line's Campfire from the bucket's project dock first and the account-wide listing second, each cached ten minutes on the `Client` every `AccountClient` derives from and refreshed on a miss no more than once per 30 s, under one candidate budget; and reports "unresolved" only when every visible candidate answered 404. The composite's own verdicts travel as the typed cause of the returned `Error` — `RecordingSummaryError::of(&error)` — so `recording_unresolved` stays distinct from a read's `forbidden`/`not_found` and from `campfire_discovery_incomplete` without minting a taxonomy code (SPEC §6's set is closed). Mentions are read and written by `basecamp_sdk::mentions`, decoding the sgid envelope structurally (Marshal or JSON, both Rails layouts; unsigned). Placement is `rust/basecamp-sdk/src/services/recordings.rs` and `.../comments.rs` (§18 rule 5), with the discovery cache in `.../campfire_index.rs`. The Rust conformance runner executes every case of `conformance/tests/recording_summary.json`, so its `RUST_SKIPS` is empty. The nested decode card 39 settled is reproduced here by construction rather than by hand: every routed read deserializes the whole typed model, so a wrong-typed member the projection never reads fails the read exactly as it does in the reference, and the top-level residue the three map-based ports declare does not exist in this port. |

**The composite's own verdicts, and the coarse code each is classified under.**
A verdict is not an HTTP answer, so the identity — the sentinel, the exception
class, the `kind`/`reason` token — is what a consumer branches on, and §6's
closed taxonomy is not where it lives. But a CLI still turns something into an
exit status, and the ports reached that answer independently and diverged:
`campfire_discovery_incomplete` shipped as `api_error` in Python, Ruby, Rust and
TypeScript (exit 7) and as `usage` in Kotlin (exit 1), for the same condition,
with all 40 fixture cases green in every one of them — because the fixture
pinned `errorType` and said nothing about the code. Card 40 settled it:

| identity | code | exit | retryable |
|----------|------|------|-----------|
| `no_recording_type` | `usage` | 1 | no |
| `unknown_recording_type` | `usage` | 1 | no |
| `recording_unresolved` | `not_found` | 2 | no |
| `campfire_discovery_incomplete` | `usage` | 1 | no |
| `bucket_mismatch` | `usage` | 1 | no |
...
`usage` for a bucket mismatch is the second row the ports had to be brought
together on (card 41, after Rust shipped `not_found` and the other four
`usage`). `not_found` says the recording is not there, which is false: the read
FOUND it, in another bucket, and returned it, so nothing is absent. What failed
is the caller's pointer, which named a bucket the recording is not in — and that
is what `usage` means everywhere else in the taxonomy. Four ports already agreed,
but the argument is what settles it.
...
same answer without the verdict becoming an HTTP-shaped error. Both are now
total over the five verdicts: in Swift the accessors are non-optional, so a
verdict added later must be classified or the build fails.
...
deduplicates by person id reintroduces that defect. The comparison is made
on the attribute's DECODED value, and the decoder's fidelity to the
reference is load-bearing for it: a port can implement the exact-match rule
faithfully, hand the attribute to its platform's HTML5 unescaper, and still
be exploitable — HTML5 drops a C0 control character to the empty string, so
`sgid="<real sgid>&#1;"` in caller-supplied content unescapes to exactly the
real sgid and suppresses the mention (Python's port was, until fixed). The
reference emits the character, so the value differs and the mention is
written; a port's unescaper must do the same, or compare the raw attribute.
