...
| 35 | `35-srv2-dimensions-carry-through-the-epoch-resume.json` | the srv2 dimensions `performers` / `exclude_performers` / `actor_types` ride the subscription identifier and the entry poll's exact query, comma-joined; a 410 epoch gap under that filter set, accepted, follows the served `since=<epoch_after_id>` resume verbatim — filters and all, never `since=now` — as a position-resume entry whose first page delivers and saves |
...
| Subscribe identifier literals: channel `EventsChannel`, param spellings `types`/`buckets`/`creators`/`performers`/`exclude_performers`/`actor_types`, comma-joined values | 1 | channel: every `expectSubscribe`; `types` spelling: 01 (its `expectSubscribe` pins `params` explicitly, single-valued); `performers`/`exclude_performers`/`actor_types` spellings + comma-joining: 35 (on the identifier, from the raw frame; on the poll, only that the dimensions reach the seam — the driver derives the query, and the wire spelling there is tier 1's); `buckets`/`creators` spellings: no PR-2 fixture — pinned at PR-4 (fixture 15, whose retransmit case also pins byte-identity of the identifier) |
...
| 410 body keys `epoch_after_id` / `resume` | 1 | 16, 23, 25, 27, 35 (feed, with `epoch_after_id`); 33 (inbox, `resume` only) |
