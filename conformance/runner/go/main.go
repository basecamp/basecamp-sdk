	...
	case "CreateTemplatification":
		...
		templatification, err := account.Templates().CreateTemplatification(ctx, bucketID, recordingID,
			&basecamp.CreateTemplatificationRequest{
				...
				CopyComments:      getOptBoolParam(tc.RequestBody, "copy_comments"),
				CopyAssignments:   getOptBoolParam(tc.RequestBody, "copy_assignments"),
				MoveCardsToTriage: getOptBoolParam(tc.RequestBody, "move_cards_to_triage"),
			})
		if err != nil {
			return operationResult{err: err}
		}
...
// getOptBoolParam keeps "absent" distinct from "present and false", which is the
// distinction a plain bool cannot carry.
func getOptBoolParam(params map[string]interface{}, key string) *bool {
	if _, ok := params[key]; !ok {
		return nil
	}
	b := getBoolParam(params, key)
	return &b
}
