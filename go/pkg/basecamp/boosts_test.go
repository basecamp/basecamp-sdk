...
func TestCreateRecordingBoost_EmptyContent(t *testing.T) {
	...
	_, err := svc.CreateRecording(context.Background(), 2, "")
	if err == nil {
...
func TestCreateEventBoost_EmptyContent(t *testing.T) {
	svc := newTestBoostsService()
	_, err := svc.CreateEvent(context.Background(), 2, 3, "")
	if err == nil {
		t.Fatal("expected error for empty content")
	}
	apiErr, ok := errors.AsType[*Error](err)
...
func TestBoostsService_ListRecording(t *testing.T) {
	...
	svc := testBoostsServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path != "/99999/recordings/200/boosts.json" {
	...
	result, err := svc.ListRecording(context.Background(), 200, nil)
	if err != nil {
...
func TestBoostsService_ListRecording_Pagination(t *testing.T) {
	fixture := loadBoostsFixture(t, "list.json")
	...
	svc := testBoostsServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		if requestCount == 1 {
			...
			w.Header().Set("Link", fmt.Sprintf(`<%s/99999/recordings/200/boosts.json?page=2>; rel="next"`, "http://"+r.Host))
			w.WriteHeader(200)
			w.Write(fixture)
		} else {
	...
	result, err := svc.ListRecording(context.Background(), 200, &BoostListOptions{Limit: -1})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	// Verify last boost from page 2
	if len(result.Boosts) >= 3 && result.Boosts[2].ID != 1069479502 {
		t.Errorf("expected third boost ID 1069479502, got %d", result.Boosts[2].ID)
	}
}

func TestBoostsService_ListRecording_SinglePage(t *testing.T) {
	fixture := loadBoostsFixture(t, "list.json")
	svc := testBoostsServer(t, func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("X-Total-Count", "100")
		w.Header().Set("Content-Type", "application/json")
		// Include a Link header that should NOT be followed in single-page mode
		w.Header().Set("Link", fmt.Sprintf(`<%s/99999/recordings/200/boosts.json?page=2>; rel="next"`, "http://"+r.Host))
		w.WriteHeader(200)
		w.Write(fixture)
	})

	result, err := svc.ListRecording(context.Background(), 200, &BoostListOptions{Page: 1})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	// Should return only the first page despite Link header
	if len(result.Boosts) != 2 {
...
func TestBoostsService_Get(t *testing.T) {
	...
	svc := testBoostsServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path != "/99999/boosts/500" {
			t.Errorf("unexpected path: %s", r.URL.Path)
		}
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(200)
	...
	boost, err := svc.Get(context.Background(), 500)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if boost.ID != 1069479500 {
...
func TestBoostsService_CreateRecording(t *testing.T) {
	...
	boost, err := svc.CreateRecording(context.Background(), 200, "🎉")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if boost.ID != 1069479500 {
...
func TestBoostsService_ListEvent(t *testing.T) {
	...
	svc := testBoostsServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path != "/99999/recordings/200/events/300/boosts.json" {
			t.Errorf("unexpected path: %s", r.URL.Path)
		}
	...
	result, err := svc.ListEvent(context.Background(), 200, 300, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(result.Boosts) != 2 {
...
func TestBoostsService_CreateEvent(t *testing.T) {
	...
	boost, err := svc.CreateEvent(context.Background(), 200, 300, "👍")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if boost.ID != 1069479500 {
