...
// checkContinuation validates a continuation or resume URL against the
// configured base origin. It returns nil when the URL may be followed, else a
// Terminal(invalid_continuation) error — no request is issued to the failing
// URL, there is no retry and no handler, and the rejected URL is carried
// redacted to its origin (a hostile continuation's path and query are exactly
// what must not be echoed).
func (l *loop) checkContinuation(pageURL string) *TerminalError {
