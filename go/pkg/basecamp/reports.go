...
func (s *ReportsService) AssignablePeople(ctx context.Context) ([]Person, error) {
...
func (s *ReportsService) AssignedTodos(ctx context.Context, personID int64, opts *AssignedTodosOptions) (*AssignedTodosResponse, error) {
	if err := s.client.RequireAccount(); err != nil {
		return nil, err
	}
...
func (s *ReportsService) OverdueTodos(ctx context.Context) (*OverdueTodosResponse, error) {
	if err := s.client.RequireAccount(); err != nil {
		return nil, err
	}
...
func (s *ReportsService) UpcomingSchedule(ctx context.Context, startDate, endDate string) (*UpcomingScheduleResponse, error) {
	if err := s.client.RequireAccount(); err != nil {
		return nil, err
	}
	...
	if startDate != "" || endDate != "" {
		...
		if startDate != "" {
			...
			if _, err := types.ParseDate(startDate); err != nil {
				return nil, ErrUsage("window_starts_on must be in YYYY-MM-DD format")
			}
			params.WindowStartsOn = startDate
		}
		if endDate != "" {
			// Validate date format
			if _, err := types.ParseDate(endDate); err != nil {
				return nil, ErrUsage("window_ends_on must be in YYYY-MM-DD format")
			}
