 ...
 */
private fun summarizeTemplatification(templatification: Templatification): JsonElement = buildJsonObject {
    put("id", templatification.id)
    put("status", templatification.status)
    templatification.destinationTodolist?.let { put("destination_todolist_id", it.id) }
    templatification.destinationCardTable?.let { put("destination_card_table_id", it.id) }
}
