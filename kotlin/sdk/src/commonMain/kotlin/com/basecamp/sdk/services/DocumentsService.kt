...
class DocumentsService(client: AccountClient) :
     ...
     * `DocumentsServiceTest.updateNormalizesADecodeFailure` asserts the marker
     ...
     * GET's decode: an auth strategy that classifies its own JSON failure that
     * way has its exception propagated untouched by `BasecampHttpClient`, and
     * would arrive here relabelled as a malformed document — for a request that
     * was never sent (#730).
     *
