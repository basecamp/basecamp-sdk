...
class ApiConstructorSurfaceTest {
     ...
     * change since is the trailing `retryAfterSeconds: Int?` (#775), which
     * carries a 5xx's Retry-After to the caller and is the last parameter so
     * every earlier positional call still lines up.
     *
    ...
    fun thePublicConstructorKeepsItsShape() {
        ...
        assertEquals(
            listOf(listOf("String", "Integer", "String", "boolean", "String", "Throwable", "Integer")),
            signatures,
            "exactly one Java-selectable constructor is expected: #751's shape plus the trailing retryAfterSeconds",
        )
    }
