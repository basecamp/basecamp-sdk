...
class HttpClient:
    ...
    def get(self, url: str, *, params: dict | None = None, operation: str | None = None) -> httpx.Response:
        url = self._build_url(url)
        return self._request_with_retry("GET", url, params=params, operation=operation)
    ...
    def request_multipart(
        ...
        *,
        ...
        if operation and self._is_retryable_operation(operation):
            return self._request_with_retry(method, url, files=files, operation=operation)
        return self._single_request(method, url, files=files)

    def get_download(self, url: str) -> httpx.Response:
    ...
    def _mutation(
        ...
        if operation and self._is_retryable_operation(operation):
            return self._request_with_retry(method, url, json_body=json_body, operation=operation)
        ...
        *,
        ...
        while True:
            ...
            try:
                ...
                # A refreshable 401 is replayed by the loop, not inside
                # _single_request, so the replay costs an attempt like any
                # other. max_retries is a TOTAL attempt count: a cap of one
                # means one request on the wire, refresh or no refresh.
