...
class BoostsServiceTest < Minitest::Test
  ...
  def test_get_boost
    stub_get("/12345/buckets/100/boosts/42", response_body: sample_boost(id: 42))

    boost = @account.boosts.get_boost(project_id: 100, boost_id: 42)
  ...
  def test_delete_boost
    stub_delete("/12345/buckets/100/boosts/42")

    result = @account.boosts.delete_boost(project_id: 100, boost_id: 42)

    assert_nil result
  end

  def test_list_recording_boosts
    stub_get("/12345/buckets/100/recordings/200/boosts.json",
             response_body: [ sample_boost(id: 1), sample_boost(id: 2) ])

    boosts = @account.boosts.list_recording_boosts(project_id: 100, recording_id: 200).to_a
  ...
  def test_create_recording_boost
    stub_post("/12345/buckets/100/recordings/200/boosts.json",
              response_body: sample_boost(id: 99))

    boost = @account.boosts.create_recording_boost(project_id: 100, recording_id: 200, content: "🔥")

    assert_equal 99, boost["id"]
  end

  def test_list_event_boosts
    stub_get("/12345/buckets/100/recordings/200/events/300/boosts.json",
             response_body: [ sample_boost(id: 5) ])

    boosts = @account.boosts.list_event_boosts(project_id: 100, recording_id: 200, event_id: 300).to_a
  ...
  def test_create_event_boost
    stub_post("/12345/buckets/100/recordings/200/events/300/boosts.json",
              response_body: sample_boost(id: 77))

    boost = @account.boosts.create_event_boost(
      project_id: 100, recording_id: 200, event_id: 300, content: "👍"
    )
