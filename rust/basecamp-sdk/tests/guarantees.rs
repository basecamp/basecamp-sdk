...
fn the_route_table_is_complete_and_consistent() {
    ...
    assert_eq!(ROUTES.len(), 270);
    assert_eq!(basecamp_sdk::metadata::OPERATIONS.len(), 270);
    let mut ids: Vec<&str> = ROUTES.iter().map(|route| route.id).collect();
    ids.dedup();
    assert_eq!(ids.len(), 270, "operation ids are unique");
    for route in ROUTES {
    ...
    assert_eq!(basecamp_sdk::OPERATION_COUNT, 270);
}
