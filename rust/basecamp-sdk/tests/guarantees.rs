...
fn the_route_table_is_complete_and_consistent() {
    ...
    assert_eq!(ROUTES.len(), 264);
    assert_eq!(basecamp_sdk::metadata::OPERATIONS.len(), 264);
    let mut ids: Vec<&str> = ROUTES.iter().map(|route| route.id).collect();
    ids.dedup();
    assert_eq!(ids.len(), 264, "operation ids are unique");
    for route in ROUTES {
    ...
    assert_eq!(basecamp_sdk::OPERATION_COUNT, 264);
}
