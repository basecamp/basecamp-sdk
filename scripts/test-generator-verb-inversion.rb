...
# restore the walks below to the ORIGINAL five-verb list and 10 of these 21 cases
# fail; restore them to the EIGHT verbs OpenAPI 3.1 names and 3 still fail —
# OpenAPI 3.2's `query`, and the two cases where a field that is neither a known
# non-operation field nor a readable operation is stepped over in silence. That
# gap is the whole argument for identifying an operation by what it is NOT, so
...
# walk and bound but need a JVM, a Swift toolchain, cargo, or an npm install to
# run; their drift checks cover them in their own CI jobs, and the mutation was
# reproduced there by hand rather than here on every push.
#
...
check("1. a GET alone still generates") do
...
check("2. a HEAD operation stops the run and names it") do
...
check("3. a HEAD beside a GET stops the run rather than silently emitting one of two") do
...
check("4. OpenAPI 3.2's `query` verb stops the run") do
...
check("5. non-operation path-item fields are skipped, not treated as operations") do
  status, output, _emitted, bodies = ruby_services(spec({ "/{accountId}/widgets.json" => {
...
check("6. a field that is neither known nor an operation object is named, not skipped") do
...
check("7. a $ref path item is refused rather than read as empty") do
...
check("8. emission order follows the declaration, not the document") do
...
check("9. the bound is read from spec/generated-verbs.json, not a private literal") do
...
check("10. a HEAD operation's retry metadata is extracted") do
...
check("11. an unreadable path-item field is named rather than skipped") do
...
check("12. a GET alone still generates") do
  status, output, emitted, = python_services(spec({ "/{accountId}/widgets.json" => { "get" => operation("ListWidgets") } }))
  [status.zero? && !emitted.empty?, "exit #{status}, files #{emitted.inspect}: #{output}"]
end

check("13. a HEAD operation stops the run and names it") do
  status, output, = python_services(spec({ "/{accountId}/widgets.json" => { "head" => operation("HeadWidgets") } }))
  [status != 0 && output.include?("HEAD /{accountId}/widgets.json") && output.include?("HeadWidgets"),
   "exit #{status}: #{output}"]
end

check("14. a HEAD beside a GET stops the run rather than silently emitting one of two") do
...
check("15. non-operation path-item fields are skipped, not treated as operations") do
  status, output, _emitted, bodies = python_services(spec({ "/{accountId}/widgets.json" => {
...
check("16. a $ref path item is refused rather than read as empty") do
  status, output, = python_services(spec({ "/{accountId}/widgets.json" => { "$ref" => "#/components/pathItems/Widgets" } }))
  [status != 0 && output.include?("$ref"), "exit #{status}: #{output}"]
end

check("17. the bound is read from spec/generated-verbs.json, not a private literal") do
...
check("18. a HEAD operation's retry metadata is extracted") do
...
check("19. a HEAD operation reaches url-routes.json rather than being dropped") do
...
check("20. non-operation path-item fields do not become routes") do
