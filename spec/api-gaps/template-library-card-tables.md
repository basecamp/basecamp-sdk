...
smithy_refs:
  ...
  - Project
bc3_refs:
  introduced_in: "BC3 #13312: route move (48489ed5c8), card table templates through the API (0ecc540a12), alias re-documentation (49e616964d), and the to-do list create plus templatification docs (37a19aea42)"
  routes:
...
Six operations on the Templates service, plus one extension to the existing copy:

- `GetTemplateLibraryTodolists`: `GET /template_library/todolists.json`, the
  kind-explicit address for the library read. It replaces `GetTemplateLibrary`,
  which is deleted rather than deprecated, per the SPEC.md policy that a rename
  ships without an alias. The old address still answers with a redirect but is
  no longer modelled.
...
- `CreateTemplateLibraryTodolist`: `POST /template_library/todolists.json`
  takes `name` and an optional `description`, returning `201` with the template.
- `CreateTemplatification` / `GetTemplatification`:
  `POST` and `GET /buckets/{id}/recordings/{id}/templatifications`. Lifts an
  existing to-do list or card table into the library, asynchronously, with its
  own status resource. Every attribute is optional and a bodyless request is
  valid. A completed result carries `destination_todolist` or
  `destination_card_table` and never a destination parent, because the library
  is the only destination it can have.
- `CreateTemplateLibraryCopy` / `GetTemplateLibraryCopy`: unchanged addresses.
  One endpoint serves both kinds, and a completed copy reports
  `destination_card_table` beside the existing `destination_todolist`. The copy
  request gains an optional `destination_project_id`: Basecamp resolves the
  container from the template's kind, so a caller names a project rather than a
  dock or to-do set. `destination_parent_id` still names a container directly
  and takes precedence when both are sent.

No project field is added. An earlier design exposed the dock's own id so a
caller could name it as a copy destination; resolving the container server-side
replaced it, and nothing on the project changes.
