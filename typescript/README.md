...
| `campfire_discovery_incomplete` | `usage` | Candidates were left unsearched, so nothing can be reported absent. `usage` is the one coarse code no HTTP response can produce, so this verdict can never be read back as a constituent read's own answer. Non-retryable: both reasons are deterministic for the same account state. The merged ports had split (`api_error` here, in Python, Ruby and Rust; `usage` in Kotlin), and [card 40](https://app.basecamp.com/2914079/buckets/48699913/card_tables/cards/10308122086) settled it for all seven; the shared fixture now pins the code, not just the identity. |
