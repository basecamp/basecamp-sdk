...
| `bucket_mismatch` | `usage` | The read returned a recording from a different bucket than the pointer named — the caller's argument, not the API misbehaving. The merged ports still split here: `usage` in Python, Ruby, Kotlin and TypeScript (exit 1), `not_found` in Rust (exit 2). Go and Swift classify every other verdict but leave this one unclassified until it is settled. |
