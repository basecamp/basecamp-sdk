...
| `recording_unresolved` | `not_found` | A chat line was found under no Campfire you can currently see. Unanimous across every port. |
| `campfire_discovery_incomplete` | `usage` | Candidates were left unsearched, so nothing can be reported absent. `usage` is one of only three coarse codes no HTTP response can produce (with `network` and `ambiguous`), and the one of those three that also describes a call the SDK declined to complete, so this verdict can never be read back as a constituent read's own answer. Non-retryable: both reasons are deterministic for the same account state. The merged ports had split (`api_error` here, in Python, Ruby and Rust; `usage` in Kotlin), and [card 40](https://app.basecamp.com/2914079/buckets/48699913/card_tables/cards/10308122086) settled it for all seven; the shared fixture now pins the code, not just the identity. |
| `bucket_mismatch` | `usage` | The read returned a recording from a different bucket than the pointer named — the caller's argument, not the API misbehaving. Not `not_found`: the read FOUND the recording, in another bucket, and returned it, so nothing is absent. The merged ports had split (`not_found` in Rust, exit 2; `usage` in the other four, exit 1), and [card 41](https://app.basecamp.com/2914079/buckets/48699913/card_tables/cards/10308966794) settled it for all seven. |
