...
export class MessageTypesService extends BaseService {
   ...
   * ```
   */
  async list(bucketId: number, options?: ListMessageTypeOptions): Promise<ListResult<MessageType>> {
    return this.requestPaginated(
      ...
      () =>
        this.client.GET("/buckets/{bucketId}/categories.json", {
          params: {
            path: { bucketId },
          },
        })
   ...
   * ```
   */
  async create(bucketId: number, req: CreateMessageTypeRequest): Promise<MessageType> {
    if (!req.name) {
    ...
    const response = await this.request(
      ...
      () =>
        this.client.POST("/buckets/{bucketId}/categories.json", {
          params: {
            path: { bucketId },
          },
          body: {
   ...
   * ```
   */
  async get(bucketId: number, typeId: number): Promise<MessageType> {
    const response = await this.request(
      {
        service: "MessageTypes",
      ...
      () =>
        this.client.GET("/buckets/{bucketId}/categories/{typeId}", {
          params: {
            path: { bucketId, typeId },
          },
        })
    );
    return response;
   ...
   * ```
   */
  async update(bucketId: number, typeId: number, req: UpdateMessageTypeRequest): Promise<MessageType> {
    const response = await this.request(
      {
        service: "MessageTypes",
      ...
      () =>
        this.client.PUT("/buckets/{bucketId}/categories/{typeId}", {
          params: {
            path: { bucketId, typeId },
          },
          body: {
   ...
   * ```
   */
  async delete(bucketId: number, typeId: number): Promise<void> {
    await this.request(
      {
        service: "MessageTypes",
      ...
      () =>
        this.client.DELETE("/buckets/{bucketId}/categories/{typeId}", {
          params: {
            path: { bucketId, typeId },
          },
        })
